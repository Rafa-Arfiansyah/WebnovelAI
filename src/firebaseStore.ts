...
export const firebaseStore = {
  ...
  async syncAllFromCloud(userId: string): Promise<{
    ...
    // Helper to get local data in case of offline fallback
    const getLocalData = () => {
      ...
      return {
        projects: localProjectsStr ? JSON.parse(localProjectsStr) : [],
        chapters: localChaptersStr ? JSON.parse(localChaptersStr) : [],
        characters: localCharactersStr ? JSON.parse(localCharactersStr) : [],
        locations: localLocationsStr ? JSON.parse(localLocationsStr) : []
      };
    };

    // If we are offline, fall back directly to local data to prevent hanging/errors
    if (typeof navigator !== "undefined" && !navigator.onLine) {
      console.warn("Running in offline mode. Loading cached local data.");
      return getLocalData();
    }
    ...
    try {
      ...
      console.warn("Cloud sync encountered a network error/timeout. Falling back to local cache.", error);
      return getLocalData();
    }
  },
